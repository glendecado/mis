...
$total = function () {
    ...
    $this->completionRate = $this->totalAssignedRequests > 0 ? round(($this->totalRequestsCompleted / $this->totalAssignedRequests) * 100, 2) : 0;
};
...
$techStaffMetrics = function () {
    ...
    switch ($date) {
        case 'today':
            $request = Request::where('assigned_requests.technicalStaff_id', $id)->join('assigned_requests', 'assigned_requests.request_id', '=', 'requests.id')->whereDate('assigned_requests.created_at', Carbon::today())->select(DB::raw("strftime('%Y-%m-%d %H', assigned_requests.created_at) as date"), DB::raw('COUNT(*) as total_assigned_requests'), DB::raw('SUM(CASE WHEN requests.status = "resolved" THEN 1 ELSE 0 END) as total_requests_resolved'))->groupBy(DB::raw("strftime('%Y-%m-%d %H', assigned_requests.created_at)"))->orderBy(DB::raw("strftime('%Y-%m-%d %H', assigned_requests.created_at)"), 'ASC');
            break;

        case 'this_week':
            $request = Request::where('assigned_requests.technicalStaff_id', $id)->where('assigned_requests.created_at', '>=', Carbon::now()->subWeek())->join('assigned_requests', 'assigned_requests.request_id', '=', 'requests.id')->select(DB::raw("strftime('%Y-%m-%d', assigned_requests.created_at) as date"), DB::raw('COUNT(*) as total_assigned_requests'), DB::raw('SUM(CASE WHEN requests.status = "resolved" THEN 1 ELSE 0 END) as total_requests_resolved'))->groupBy(DB::raw("strftime('%Y-%m-%d', assigned_requests.created_at)"))->orderBy(DB::raw("strftime('%Y-%m-%d', assigned_requests.created_at)"), 'ASC');
            break;

        case 'this_month':
            $request = Request::where('assigned_requests.technicalStaff_id', $id)->join('assigned_requests', 'assigned_requests.request_id', '=', 'requests.id')->select(DB::raw("strftime('%Y', assigned_requests.created_at) as week"), DB::raw("strftime('%W', assigned_requests.created_at) as date"), DB::raw('COUNT(*) as total_assigned_requests'), DB::raw('SUM(CASE WHEN requests.status = "resolved" THEN 1 ELSE 0 END) as total_requests_resolved'))->groupBy(DB::raw("strftime('%Y', assigned_requests.created_at)"), DB::raw("strftime('%W', assigned_requests.created_at)"))->orderBy(DB::raw("strftime('%Y', assigned_requests.created_at)"), 'ASC')->orderBy(DB::raw("strftime('%W', assigned_requests.created_at)"), 'ASC');
            break;
    }
