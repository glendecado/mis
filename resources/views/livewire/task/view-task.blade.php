                ...
                <tr class="border-b border-gray-300 hover:bg-gray-100">
                    ...
                    <td class="py-3 px-6">
                        ...
                        <button
                            ...
                            wire:loading.class="bg-transparent border-none shadow-none hover:bg-transparent"
                            class="bg-blue-900 text-white py-2 px-4 rounded shadow-md  hover:bg-blue-100 relative h-10 w-32 "
                            @click="$dispatch('add-task', {request_id: '{{ $id ?? '' }}', tech_id: '{{ $tech->user->id }}' }); 
            $dispatch('view-task'); 
            $dispatch('view-request')">

                            <span wire:loading.remove>Assigned</span>
                            <span wire:loading class="absolute inset-0 flex items-center justify-center bg-transparent">
                        ...
                        <button
                            wire:loading.attr="disabled"
                            wire:loading.class="bg-transparent border-none shadow-none hover:bg-transparent"
                            class="bg-gray-200 text-gray-800 py-2 px-4 rounded shadow-md  hover:bg-blue-100 relative h-10 w-32 "
                            @click="$dispatch('add-task', {request_id: '{{ $id ?? '' }}', tech_id: '{{ $tech->user->id }}' }); 
            $dispatch('view-task'); 
            $dispatch('view-request')">
